...
class WhiteBallDetector:
    def __init__(self):
        # ========= 白色 HSV 范围 =========
        # 根据你测到的白球数据：
        # H≈0, S≈0, V≈109~151，局部最高到180+
        self.lower_white = np.array([0, 0, 95], dtype=np.uint8)
        self.upper_white = np.array([20, 20, 255], dtype=np.uint8)
        ...
        self.min_wh_ratio = 0.6
        self.max_wh_ratio = 1.40
    ...
    def detect(self, frame_bgr):
        ...
        mask_raw = cv2.inRange(hsv, self.lower_white, self.upper_white)
        ...
        if len(candidates) > 0:
            ...
            cv2.putText(
                frame_vis,
                f"BEST WHITE BALL r={br:.1f} c={best_ball['circularity']:.2f}",
                (bx1, max(by1 - 8, 20)),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6,
                (0, 0, 255),
...
class WhiteBallBestDebugNode(Node):
    def __init__(self):
        super().__init__('white_ball_best_debug_node')

        self.bridge = CvBridge()
        self.detector = WhiteBallDetector()
        ...
        self.last_log_ns = 0
        self.get_logger().info('white_ball_best_debug_node started')
    ...
    def image_callback(self, msg: Image):
        ...
        cv2.imshow('white_ball_frame_vis', result['frame_vis'])
        cv2.imshow('white_ball_roi_vis', result['roi_vis'])
        cv2.imshow('white_ball_mask_raw', result['mask_raw'])
        cv2.imshow('white_ball_mask_morph', result['mask_morph'])
        cv2.imshow('white_ball_contour_vis', result['contour_vis'])
        ...
        if now_ns - self.last_log_ns > int(0.6 * 1e9):
            ...
            self.get_logger().info("========== WHITE BALL BEST DEBUG BEGIN ==========")
            ...
            if result["best_ball"] is not None:
                ...
                self.get_logger().info("WHITE BALL not found")

            self.get_logger().info("========== WHITE BALL BEST DEBUG END ==========")


def main(args=None):
    rclpy.init(args=args)
    node = WhiteBallBestDebugNode()
    try:
        rclpy.spin(node)
