...
class ObstacleDashedTaskNode(Node):
    ...
    # 新增：最终对准横向黄线后，再执行两次左跳动作，然后 DONE
    ...
    def __init__(self):
        ...
        #   任务结束，发送 STOP。
        # ============================================================
        self.declare_parameter('initial_state', 'APPROACH_OBSTACLES')
        ...
        # 运动参数
        self.declare_parameter('obstacle_forward_speed', 0.30)
        self.declare_parameter('obstacle_search_forward_speed', 0.30)
        self.declare_parameter('obstacle_trigger_distance_m', 0.30)

        self.declare_parameter('obstacle_align_vy_k', 0.35)
        self.declare_parameter('obstacle_align_vy_max', 0.20)
        self.declare_parameter('obstacle_align_vy_min', 0.10)
        self.declare_parameter('obstacle_center_px_deadband', 7)

        self.declare_parameter('dashed_align_vy_k', 0.35)
        self.declare_parameter('dashed_align_vy_max', 0.20)
        self.declare_parameter('dashed_align_vy_min', 0.15)
        self.declare_parameter('dashed_center_px_deadband', 12)
        ...
        self.declare_parameter('follow_forward_speed', 0.30)
        ...
        self.declare_parameter('post_dash_forward_distance_m', 0.30)
        ...
        self.declare_parameter('align_forward_speed_far', 0.30)
        ...
        self.declare_parameter('post_hit_final_forward_speed', 0.30)
    ...
    def enter_initial_state(self):
        ...
        if self.initial_state not in valid_states:
            self.get_logger().warn(
                ...
                f"fallback to {self.APPROACH_OBSTACLES}. "
                f"valid_states={valid_states}"
            )
            self.initial_state = self.APPROACH_OBSTACLES
    ...
    def enter_state(self, new_state: str):
        ...
        if new_state == self.FINAL_LEFT_JUMP:
            # 最终左跳是阻塞动作，这里只负责进入状态；动作在 control_loop 中执行。
    ...
    def control_loop(self):
        ...
        if self.state == self.APPROACH_OBSTACLES:
            ...
            self.execute_left_jump_turn(2, self.DONE)
        ...
        if now - self.last_log_time > 0.5:
            ...
            self.get_logger().info(
                f'state={self.state} cmd=({vx:.3f},{vy:.3f},{wz:.3f}) '
